...
var Query = function(connection, model) {
  var self = this;
  this._getConnection = function (conn, callback) {
    if (conn) {
      return callback(conn);
    } else if (connection instanceof Connection) {
      return callback(connection)
    } else if (typeof connection === 'function') {
      return connection(function (err, connection) { callback(connection) });
    } else if (connection.connect) {
      return connection.connect(function (err, connection) { callback(connection) });
    }
...
Query.prototype.each = function() {
  ...
  this._getConnection(connection, function (connection) {
    connection.each(self._sqlTree(), callback, doneCallback);
  });
};
...
Query.prototype.count = function() {
  ...
  this._getConnection(connection, function (connection) {
    connection.single(self._sqlTree(), 'count', callback);
  });
};
...
Query.prototype.min = function() {
  ...
  this._getConnection(connection, function (connection) {
    connection.single(self._sqlTree(), 'min', callback);
  });
  return null;
};
...
Query.prototype.max = function() {
  ...
  this._sqlTree().columns = [ this._sqlTree().getColumnByPropertyName(field) ];
  this._getConnection(connection, function (connection) {
    connection.single(self._sqlTree(), 'max', callback);
  });
  return null;
};
...
Query.prototype.sum = function() {
  ...
  this._sqlTree().columns = [ this._sqlTree().getColumnByPropertyName(field) ];
  this._getConnection(connection, function (connection) {
    connection.single(self._sqlTree(), 'sum', function(err, sum) {
      if (err) {
        return callback(err);
      }
      if (sum === null) {
...
Query.prototype.all = function() {
  ...
  var callback = arguments[arguments.length - 1];
  this._getConnection(connection, function (connection) {
    connection.all(self._sqlTree(), callback);
  });
};
...
Query.prototype.first = function() {
  ...
  this._getConnection(connection, function (connection) {
    self.all(connection, function(err, rows) {
      if (err) {
        callback(err);
        return;
      }
...
Query.prototype.getById = function() {
  ...
  this._getConnection(connection, function (connection) {
    self.where(self.model.getIdPropertyName() + " = ?", id).first(connection, function(err, row) {
      if (err) {
        callback(err);
        return;
      }
